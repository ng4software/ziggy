...
pub fn format(allocator: std.mem.Allocator, comptime fmt: []const u8, args: anytype) std.fmt.AllocPrintError![]u8 {
...
test "format string" {
    ...
    try std.testing.expect(cmp.str(result, "Hello World"));
}
...
test "format integer" {
    ...
    const args = .{ 42 };
    const result = try format(allocator, pattern, args);
    try std.testing.expect(cmp.str(result, "Hello 42"));
}